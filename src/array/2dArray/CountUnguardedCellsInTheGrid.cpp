...
public:
    int countUnguarded(int m, int n, vector<vector<int>>& guards, vector<vector<int>>& walls) {
        ...
        for(auto g : guards) {
            ...
            // up
            int i = r - 1;
            while(i >= 0 && mat[i][c] != 'W' && mat[i][c] != 'G')  mat[i--][c] = '1';
            
            // down
            i = r + 1;
            while(i < m && mat[i][c] != 'W' && mat[i][c] != 'G') mat[i++][c] = '1';

            // left
            int j = c - 1;
            while(j >= 0 && mat[r][j] != 'W' && mat[r][j] != 'G') mat[r][j--] = '1';

            // right
            j = c + 1;
            while(j < n && mat[r][j] != 'W' && mat[r][j] != 'G') mat[r][j++] = '1';
        }
